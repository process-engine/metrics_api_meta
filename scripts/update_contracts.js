...
async function getVersionFromNpmTag(npmTag) {
  ...
  const tagsAsString = await exec('npm dist-tag ls @process-engine/metrics_api_contracts');
...
function setPersistenceApiContractsBranchVersionForPackage(packageName, versionToSet) {
  ...
  console.log(`Setting @process-engine/metrics_api_contracts for package ${packageName}`)
  ...
  packageJson.dependencies['@process-engine/metrics_api_contracts'] = versionToSet;
...
async function createCommit() {
  ...
  try {
    ...
    await exec('git commit -m ":recycle: Update versions for metrics_api_contracts"');
