...
```
meta exec "any command" --exclude metrics_api_core,metrics_api_contracts
meta exec "any command" --include-only metrics_api_core,metrics_api_contracts
```
...
```
meta exec "git flow feature start my_feature" --include-only metrics_api,metrics_api_contracts
```

* Starts the feature "my_feature" in the modules
`metrics_api` and `metrics_api_contracts`

### 3.7 Publish a feature on multiple repositories

```
meta exec "git flow feature publish my_feature" --include-only metrics_api,metrics_api_contracts
```
